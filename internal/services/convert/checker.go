...
// checkOriginalFile uses ffprobe to check if the original file contains the
// streams required by the requested output.
// It returns (true, nil) if all required streams are present.
// It returns (false, error) if a required stream is missing.
...
func (c *Service) checkOriginalFile(path string, requireVideo bool) (bool, error) {
